...
def eval_only(agent, env, logger, args, lag):
  ...
  def per_episode(ep, video_buffer):
    ...
    if 'log_success_rate' in ep.keys():
      success_rate = float(ep['log_success_rate'].astype(np.float64).sum())
      logger.add({'success_rate': success_rate}, prefix='episode')
      print(f'Episode has {length} steps and success_rate {success_rate:.1f}.')
    if 'cost' in ep.keys():
    ...
    for key, value in ep.items():
      if not args.log_zeros and key not in nonzeros:
        continue
      nonzeros.add(key)
    ...
    if 'log_image_orignal' in ep.keys():
      for i in range(ep['log_image_orignal'].shape[0]):
        groundtruth_video_list.append(ep['log_image_orignal'][i])
      save_video(
  ...
  while step < args.steps:
    driver(policy, steps=args.max_episode_steps, lag=lag.lagrange_penalty)
    if should_log(step):
