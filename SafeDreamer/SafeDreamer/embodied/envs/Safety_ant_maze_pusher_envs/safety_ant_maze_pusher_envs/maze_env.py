...
class MazeEnv(gym.Env):
  ...
  def __init__(
    ...
    # Получаем абсолютный путь к файлу в пакете
    xml_path = pkg_resources.resource_filename(
        'safety_ant_maze_pusher_envs', 
        os.path.join(MODEL_DIR, model_cls.FILE)
    )
  ...
  def reset(self, validate=False, start_point=None):
    ...
    assert (validate == start_point == False) or (validate != start_point)
    if validate:
