...
class CometeLogger:
  ...
  def __init__(self, logdir, config, pattern=r'.*'):
    ...
    import tensorflow as tf
    comet_ml.login()
    # Map SafeDreamer tasks -> Comet experiment display names
    try:
      task = config["task"] if isinstance(config, dict) else config.task
    except Exception:
      task = ""
    task = str(task)
    name_map = {
      "safeantmaze_cshape": "SafeDreamer AntMazeCshape",
      "safeantmaze_wshape": "SafeDreamer AntMazeWshape",
      "safeantmaze_pusher": "SafeDreamer Pusher",
    }
    exp_name = name_map.get(task, f"SafeDreamer {task}" if task else "SafeDreamer")
    self.experiment = comet_ml.start(project_name="ites")
    try:
      self.experiment.set_name(exp_name)
    except Exception:
      pass
    self.experiment.log_parameters(config)
    self.experiment.log_parameter("weights_path", logdir)
    self.experiment.log_parameter("experiment_name", exp_name)
  ...
  def __call__(self, summaries):
    ...
    for step, name, value in summaries:
      if len(value.shape) == 0 and self._pattern.search(name):
        ...
        #bystep[step][name] = self.experiment.log_histogram_3d(value)
        self.experiment.log_histogram_3d(value)
      elif len(value.shape) == 2:
        value = np.clip(255 * value, 0, 255).astype(np.uint8)
        value = np.transpose(value, [2, 0, 1])
        #bystep[step][name] = self.experiment.log_image(value)
        self.experiment.log_image(value)
      elif len(value.shape) == 3:
        value = np.clip(255 * value, 0, 255).astype(np.uint8)
        value = np.transpose(value, [2, 0, 1])
        #bystep[step][name] = self.experiment.log_image(value)
        ...
        if np.issubdtype(value.dtype, np.floating):
          value = np.clip(255 * value, 0, 255).astype(np.uint8)
        #bystep[step][name] = self.experiment.log_image(value, fps=40)
        #bystep[step][name] = self.experiment.log_image(value)
        #self.experiment.log_image(value)
        #self.experiment.log_video(
        #            value,
        #            name=name,
        #            step=step,
        #            #fps=25,  # Или ваш FPS
        ...
        for frame in value:
            # Конвертируем из CHW в HWC для OpenCV
            frame = np.transpose(frame, [1, 2, 0])
            out.write(frame)
        
        out.release()
        
        # Логируем видео в Comet
        self.experiment.log_video(video_filename, name=name, step=step, overwrite=True)
