...
class BatchOneHotAction(base.Wrapper):
  ...
  def step(self, action):
    non_reset_mask = ~action['reset']
    actions = action[self._key][non_reset_mask]
    """
    if non_reset_mask.any():
        non_reset_actions = action[self._key][non_reset_mask]
        
        # Проверки one-hot
        assert (non_reset_actions.min(axis=-1) == 0.0).all(), "Actions min != 0"
        assert (non_reset_actions.max(axis=-1) == 1.0).all(), "Actions max != 1"
        assert np.allclose(non_reset_actions.sum(axis=-1), 1.0), "Actions sum != 1"
    """
    # Проверки one-hot
    assert (actions.min(axis=-1) == 0.0).all(), "Actions min != 0"
    assert (actions.max(axis=-1) == 1.0).all(), "Actions max != 1"
    assert np.allclose(actions.sum(axis=-1), 1.0), "Actions sum != 1"

    # Конвертируем one-hot в индексы
    indices = np.argmax(action[self._key], axis=-1)  # Форма (2,)
...
class ExpandScalars(base.Wrapper):
  ...
  def __init__(self, env):
    ...
    for key, space in self.env.obs_space.items():
      if space.shape == () and key != 'reward' and key != 'cost' and not space.discrete and not "log" in key:
