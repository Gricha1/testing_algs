...
class JAXAgent(embodied.Agent):
  ...
  def report_eval(self, data):
    ...
    rng = self._next_rngs(self.train_devices)
    mets, _ = self._report_eval(self.varibs, rng, data)
    mets = self._convert_mets(mets, self.train_devices)
    return mets
  ...
  def _transform(self):
    ...
    self._report_eval = nj.pure(self.agent.report_eval)
    ...
    if len(self.train_devices) == 1:
      ...
      self._report_eval = nj.jit(self._report_eval, **kw)
      ...
      self._report_eval = nj.pmap(self._report_eval, 'i', **kw)
