...
class WorldModel(nj.Module):
  ...
  def imagine(self, policy, start, horizon, use_planner=False):
    ...
    if use_planner:
      planner_keys = []
      if 'action_mean' in start:            # непрерывные действия
          planner_keys += ['action_mean', 'action_std']
      if 'action_logits' in start:          # дискретные действия
          planner_keys += ['action_logits']
      planner_keys += ['lagrange_multiplier', 'penalty_multiplier']
      keys += planner_keys
      start = {k: v for k, v in start.items() if k in keys}
      start['action'] = policy(start,0)
      def step(prev, current_horizon): # add the current_horizon
        prev = prev.copy()
        planner_state = {k: prev[k] for k in planner_keys if k in prev}
        state = self.rssm.img_step(prev, prev.pop('action'))
        return {
                  **state,
                  **planner_state,
                  'action': policy(prev, current_horizon + 1),  # следующее действие
              }
    else:
      start = {k: v for k, v in start.items() if k in keys}
