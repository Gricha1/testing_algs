...
def make_logger(parsed, logdir, step, config):
  ...
  outputs = [
      ...
      embodied.logger.JSONLOutput(
          logdir, 'scores.jsonl',
          'episode/score|episode/cost|episode/success_rate|episode/length|episode/SR'),
      embodied.logger.TensorBoardOutput(logdir),
      #embodied.logger.WandBOutput(logdir, config),
      #embodied.logger.MLFlowOutput(logdir, config),
  ]
  # Comet hangs eval on upload confirmation; skip for eval_only or SKIP_COMET=1.
  skip_comet = (
      str(getattr(config.run, 'script', '')) == 'eval_only'
      or os.environ.get('SKIP_COMET', '0') == '1'
  )
  if not skip_comet:
    outputs.append(embodied.logger.CometeLogger(logdir, config))
...
def make_env(config, **overrides):
  ...
  ctor = {
      ...
      'craftext': 'embodied.envs.craftext:Craftext',
      'batchcraftext': 'embodied.envs.craftext_batch:Craftext_Batch',
      'safeantmaze': 'embodied.envs.safeantmaze:SafeAntMaze',
  }[suite]
...
def wrap_env(env, config):
  ...
  for name, space in env.act_space.items():
    if name == 'reset':
      ...
      if suite == "batchcraftext":
        env = wrappers.BatchOneHotAction(env, name)
      else:
