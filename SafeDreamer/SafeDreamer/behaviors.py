...
class PIDPlanner(nj.Module):
  ...
  def descrete_policy(self, latent, state):
      def policy(latent, state):
        ...
        num_actions = int(self.act_space.shape[0])  # дискретное пространство
        if 'action_logits' in state:
            logits = jnp.roll(state['action_logits'], -1, axis=0)
            logits = logits.at[-1].set(logits[-2])
        else:
            logits = jnp.zeros((self.horizon + 1, num_actions))

        for i in range(self.iterations):
            #latent_cem = {k: jnp.repeat(v, self.num_samples, 0) for k, v in latent.items()}
            latent_cem = {
                k: jnp.repeat(v[None, ...], self.num_samples, axis=0)      #########################
                for k, v in latent.items()
            }
            latent_cem['is_terminal'] = jnp.zeros(latent_cem['deter'].shape[0])

            # Sample action sequences: shape (horizon+1, num_samples)
            probs = jax.nn.softmax(logits, axis=-1)
            rng = nj.rng()
            #actions = jax.vmap(
            #    lambda p: jax.random.categorical(rng, jnp.log(p), shape=(self.num_samples,))
            #)(probs)  # [horizon+1, num_samples]
            actions_idx = jax.vmap(                                       #########################
                lambda p: jax.random.categorical(rng, jnp.log(p),         #########################
                                                shape=(self.num_samples,)) #########################
            )(probs) 
            actions = jax.nn.one_hot(actions_idx, num_actions,         #########################
                                        dtype=jnp.float32)

            def policy(s, horizon):
                return actions[horizon]

            traj_cem = self.wm.imagine(policy, latent_cem, self.horizon, use_planner=True)

            if num_pi_trajs > 0:
                traj_combined = {k: jnp.concatenate([traj_pi[k], traj_cem[k]], axis=1) for k in traj_cem}
            else:
                traj_combined = traj_cem

            rew, ret, value = self.ac.critics['extr'].score(traj_combined)

            recon = self.wm.heads['decoder'](traj_combined)
            if self.config.use_cost:
                cost, cost_ret, cost_value = self.ac.cost_critics['extr'].score(traj_combined)
            ...
            elite_value, elite_actions = self.func_with_if_else(num_safe_traj, traj_ret_penalty, traj_cost, traj_combined)

            elite_idxs = jax.lax.top_k(elite_value, self.num_elites)[1]
            elite_actions = elite_actions[:, elite_idxs]  # [horizon+1, num_elites]

            # Update logits via empirical distribution
            one_hot_elite = elite_actions  # [horizon+1, num_elites, num_actions]
            new_probs = one_hot_elite.mean(axis=1) 
            logits = self.momentum * logits + (1 - self.momentum) * jnp.log(new_probs + 1e-8)

        final_action_idx = jnp.argmax(logits[0])                                   #########################
        final_action     = jax.nn.one_hot(final_action_idx, num_actions,            #########################
                                          dtype=jnp.float32)

        return {
            'action': final_action,
            'log_action_logits': logits,
            'log_plan_num_safe_traj': num_safe_traj,
            'log_plan_ret': ret[0].mean(axis=0),
            'log_plan_cost': traj_cost.mean(axis=0)
        }, {'action_logits': logits}
      
      return policy(latent, state)

  def descrete_policy_batch(self, latent_batch, state_batch):
    """
    latent_batch : deter (3, 512) logit (3, 48, 48) stoch (3, 48, 48), где B = 3
    state_batch  : lagrange_penalty (3,) lagrange_p (3,) lagrange_i (3,) lagrange_d (3,)
    Возвращает:
        actions : {'action': (B, act_dim), ...}
        new_states : ...
    """
    actions, new_states = jax.vmap(
        lambda lat, st: self.descrete_policy(lat, st),
        in_axes=(0, 0)
    )(latent_batch, state_batch)

    return actions, new_states

  def policy(self, latent, state):    
    if self.config.action_type == "discrete":
      return self.descrete_policy_batch(latent, state)
    else:
      return self.continous_policy_batch(latent, state)

  def continous_policy_batch(self, latent_batch, state_batch):
    """
    latent_batch : deter (3, 512) logit (3, 48, 48) stoch (3, 48, 48), где B = 3
    state_batch  : lagrange_penalty (3,) lagrange_p (3,) lagrange_i (3,) lagrange_d (3,)
    Возвращает:
        ...
        new_states : ...
    """
    # vmap применит continous_policy ко всем B средам параллельно
    actions, new_states = jax.vmap(                            #########################
        lambda lat, st: self.continous_policy(lat, st),
        in_axes=(0, 0)                                         #########################
    )(latent_batch, state_batch)
      
    return actions, new_states 

  def continous_policy(self, latent, state):
    def policy(latent, state):
      """
      ...
      num_pi_trajs = int(self.mixture_coef * self.num_samples)
      if num_pi_trajs > 0:
        latent_pi = {k: jnp.repeat(v[None, ...], num_pi_trajs, axis=0)
                      for k, v in latent.items()} 
        latent_pi['is_terminal'] = jnp.zeros(latent_pi['deter'].shape[0])
        policy = lambda s: self.ac.actor(sg(s)).sample(seed=nj.rng())
        traj_pi = self.wm.imagine(policy, latent_pi, self.horizon)
      ...
      for i in range(self.iterations):
        latent_gaus = {k: jnp.repeat(v[None, ...], self.num_samples, axis=0)
                        for k, v in latent.items()}
      ...
      a = mean[0]
      return {'action': a, 'log_action_mean': mean.mean(axis=0), 'log_action_std': std.mean(axis=0), \
              'log_plan_num_safe_traj': num_safe_traj, 'log_plan_ret': ret[0,:].mean(axis=0), \
              'log_plan_cost': traj_cost.mean(axis=0)}, {'action_mean': mean, 'action_std': std}
    return policy(latent, state)
