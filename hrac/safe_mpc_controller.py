...
import numpy as np
import torch
from hrac.optimizers import RandomOptimizer, CEMOptimizer, RCEOptimizer

class SafeMPC(object):
    optimizers = {"CEM": CEMOptimizer, "RANDOM": RandomOptimizer, "RCE": RCEOptimizer}

    def __init__(self, env, mpc_config, cost_model = None, n_ensembles=0, dynamic_model=None, device=None):
    ...
    def select_action(self, state, subgoal, evaluation=False):
        action = np.squeeze(np.array([self.act(model=self.dynamic_model, state=state, goal=subgoal)]))
        return action

    def act(self, model, state, goal):
        '''
    ...
    def cost_function(self, actions):
        ...
        with torch.no_grad():
            ...
            goal = np.repeat(self.goal.reshape(1, -1), self.popsize, axis=0) # [pop size, state dim]
            ...
            for t in range(self.horizon):
                ...
                state_next = self.model.step(state, action, deterministic=True, torch_deviced=True) #+ state
                cost = self.cost_model.predict(goal, state, action)  # compute cost
